...
class Plugin
{
    ...
    public function register_hooks(){
        ...
        if( ! is_admin() ){
            ...
            if( version_compare( $wp_version, '5.9.3', '>' ) ){
                add_filter( 'do_parse_request', array( $this->wpManager, 'customParseRequest' ), 50, 3 );
            }else{
                add_filter( 'do_parse_request', array( $this->wpManager, 'customParseRequestBefore60' ), 50, 3 );
            }
            ...
            add_filter( 'posts_where', [ $this->wpManager, 'fixPostsWhereForSearch' ], 50, 2 );
            add_filter( 'post_limits_request', [ $this->wpManager, 'addSQlComment' ], 50, 2 );
            ...
            if ( flrt_is_woocommerce() ){
                add_action( 'woocommerce_product_query', 'flrt_remove_product_query_post_clauses', 50, 2 );
        ...
        if( ! defined('FLRT_FILTERS_PRO') ) {
            ...
            add_filter( 'wpc_filter_set_default_fields', [ $this, 'addAvailableInProFields' ], 50, 2 );
    ...
    public function includeFrontJs()
    {
        ...
        $autoScrollOffset   = apply_filters( 'wpc_auto_scroll_offset', 500 );
        $wpc_mobile_width   = flrt_get_mobile_width();
        $per_page           = [];
        $applyButtonSets    = [];
        $queryOnThePageSets = [];
